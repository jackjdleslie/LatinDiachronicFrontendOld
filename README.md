...
Currently the latest build of the `master` branch is deployed via Netlify and is available at the link [latin.netlify.com](https://latin.netlify.com). You can also run locally by cloning this repository and starting a development server with `node` and `yarn`.
